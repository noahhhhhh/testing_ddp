...
#                         round(predict(model, newdata = df_temp), 5) # a simple linear regression coefficient, this part needs improvement later
        ...
        output$Plot <- renderPlot({            
            ...
            g_top <- g_top + annotate("text", x = (max_age() + 1 + age()) / 2, y = ex() * 1.05, label = paste("Years left:", ex()), size = 4, colour = "#74c3ea")
#             g_top <- g_top + scale_x_continuous(
#                 limits = c(trunc(age() / 10) * 10, max_age())
#                 , breaks = seq(
#                     from = trunc(
#                         age() / 10) * 10
#                     , to = max_age(), 5
...
#             )
            ...
            g_bottom <- g_bottom + annotate("text", x = (max_age() + 1 + age()) / 2, y = max_mx() / 1.05, label = paste("Probability of dying: ", round(mx() * 100, 2), "%"), size = 4, colour = "salmon")
